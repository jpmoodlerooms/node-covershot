...
function render(filePath, templatePath, viewData, callback) {
  ...
  if (!layout) {
    layout = fs.readFileSync(path.join(__dirname, 'layout.ejs'), 'utf8');
  }
  var template = templateCache[templatePath] || (templateCache[templatePath] = fs.readFileSync(templatePath, 'utf8'));
